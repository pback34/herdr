---
title: 集成
description: 为 Pi、OMP、Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、Qoder CLI、Qwen Code、Letta Code、Command Code、Cursor Agent CLI、MastraCode、Antigravity CLI 和 Grok CLI 安装 Herdr 集成。
---
...
| 会话身份 | Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Qoder CLI、Qwen Code、Letta Code、Command Code、Cursor Agent CLI、Hermes Agent、Antigravity CLI、Grok CLI | 集成上报用于恢复的原生会话引用。状态仍来自 Herdr 的屏幕清单检测。 |
...
Command Code 集成安装是实验性的 CLI 专用功能，不在 Settings 的集成列表或 JSON 集成 API 中提供。`herdr integration status` 将其显示为 `cmd (experimental)`。智能体检测和 `herdr agent start --kind cmd` 不受此安装方式限制，均已支持。
...
静默的 `SessionStart` 钩子会上报会话 ID,用于原生恢复。Command Code 状态仍来自 Herdr 的屏幕清单检测。

Herdr 使用 `~/.commandcode`。配置目录必须已经存在。安装会写入 `hooks/herdr-agent-session.sh`（Windows 上为 `hooks/herdr-agent-session.ps1`）,向 `settings.json` 添加 Herdr 的 `SessionStart` 条目,并在 `trusted-hooks.json` 中记录指纹,以便 Command Code 运行该钩子。卸载只移除匹配条目、指纹和托管脚本。

如果还有其他 SessionStart 钩子上报 Command Code 状态（例如社区插件）,安装会发出警告并保留该钩子。在该钩子被移除之前,这些状态上报优先于内置的屏幕清单。

Herdr 用 `cmd --session <id>` 恢复保存的 Command Code 会话（Windows 上为 `command-code --session <id>`）。
