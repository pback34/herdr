---
title: Integrations
description: Install Herdr integrations for Pi, OMP, Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Kimi Code CLI, OpenCode, Kilo Code CLI, Hermes Agent, Qoder CLI, Qwen Code, Letta Code, Command Code, Cursor Agent CLI, MastraCode, Antigravity CLI, and Grok CLI.
---
...
herdr integration uninstall cmd
...
| Session identity | Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Qoder CLI, Qwen Code, Letta Code, Command Code, Cursor Agent CLI, Hermes Agent, Antigravity CLI, Grok CLI | The integration reports native session references for restore. State still comes from Herdr's screen manifest detection. |
...
Command Code integration installation is experimental and CLI-only. It is not listed in the Settings integration screen or the JSON integration API. `herdr integration status` reports it as `cmd (experimental)`. Agent detection and `herdr agent start --kind cmd` are supported independently of this installation restriction.
...
The quiet `SessionStart` hook reports the session ID for native restore. Command Code state still comes from Herdr's screen manifest detection.

Herdr uses `~/.commandcode`. The config directory must already exist. Install writes `hooks/herdr-agent-session.sh` (`hooks/herdr-agent-session.ps1` on Windows), adds a Herdr `SessionStart` entry to `settings.json`, and records a fingerprint in `trusted-hooks.json` so Command Code will run the hook. Uninstall removes only the matching entry, fingerprint, and managed script.

If another SessionStart hook still reports Command Code state (for example the community plugin), install warns and leaves that hook in place. Those state reports outrank the bundled screen manifest until that hook is removed.

Herdr resumes stored Command Code sessions with `cmd --session <id>` (`command-code --session <id>` on Windows).
