---
title: インテグレーション
description: Pi、OMP、Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、Qoder CLI、Qwen Code、Letta Code、Command Code、Cursor Agent CLI、MastraCode、Antigravity CLI、Grok CLI 向けの Herdr インテグレーションをインストールします。
---
...
| セッション識別 | Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Qoder CLI、Qwen Code、Letta Code、Command Code、Cursor Agent CLI、Hermes Agent、Antigravity CLI、Grok CLI | インテグレーションは復元用のネイティブセッション参照を報告します。状態は引き続き Herdr のスクリーンマニフェスト検出から得られます。 |
...
Command Code インテグレーションのインストールは実験的な CLI 専用機能です。Settings のインテグレーション一覧や JSON インテグレーション API には含まれません。`herdr integration status` では `cmd (experimental)` と表示されます。エージェント検出と `herdr agent start --kind cmd` は、このインストール方法の制限とは別にサポートされています。

Command Code フックをインストールします:

```bash
herdr integration install cmd
```

静かな `SessionStart` フックは、ネイティブ復元用のセッション ID を報告します。Command Code の状態は引き続き Herdr のスクリーンマニフェスト検出から得られます。

Herdr は `~/.commandcode` を使います。設定ディレクトリはあらかじめ存在している必要があります。インストールは `hooks/herdr-agent-session.sh`（Windows では `hooks/herdr-agent-session.ps1`）を書き込み、`settings.json` に Herdr の `SessionStart` エントリを追加し、Command Code がフックを実行できるようにフィンガープリントを `trusted-hooks.json` に記録します。アンインストールは一致するエントリ、フィンガープリント、管理対象スクリプトだけを削除します。

別の SessionStart フックが依然として Command Code の状態を報告している場合（たとえばコミュニティプラグイン）、インストールは警告を表示し、そのフックはそのまま残します。そのフックが削除されるまで、これらの状態報告は同梱のスクリーンマニフェストより優先されます。

Herdr は保存された Command Code セッションを `cmd --session <id>`（Windows では `command-code --session <id>`）で resume します。
