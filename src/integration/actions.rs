...
/// Experimental Command Code install that bypasses the frozen client endpoint
/// `IntegrationTarget` enum. Fold into the agent registry when it lands.
pub(crate) fn install_experimental_command_code() -> io::Result<Vec<String>> {
    let result = install_command_code().map(|installed| {
        let mut messages = vec![
            ...
            format!(
                "trusted the command code hook in {}",
                installed.trusted_hooks_path.display()
            ),
        ];
    ...
    let outcome = if result.is_ok() { "ok" } else { "error" };
    crate::logging::integration_action("install", "cmd", outcome);
    result
}

/// Experimental Command Code uninstall counterpart.
pub(crate) fn uninstall_experimental_command_code() -> io::Result<Vec<String>> {
    let result = uninstall_command_code().map(|result| {
        let mut messages = Vec::new();
        if result.removed_hook_file {
            messages.push(format!(
        ...
        if result.updated_trusted_hooks {
            messages.push(format!(
                "removed herdr command code hook fingerprint from {}",
                result.trusted_hooks_path.display()
            ));
        } else {
            messages.push(format!(
                "no herdr command code hook fingerprint found in {}",
                result.trusted_hooks_path.display()
            ));
        }
        messages
    ...
    let outcome = if result.is_ok() { "ok" } else { "error" };
    crate::logging::integration_action("uninstall", "cmd", outcome);
    result
}
